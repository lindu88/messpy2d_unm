...
class MainWindow(QMainWindow):
    def __init__(self, controller):
        ...
        for c in controller.cam_list:
            ...
            obs = [lambda i=i, c=c: c.last_read.lines[i, :]
                   for i in range(c.cam.lines)]
            op = ObserverPlotWithControls(
                c.cam.line_names, obs, lf, x=c.disp_axis)
            dw = QDockWidget('Readings', parent=self)
            dw.setWidget(op)
            dock_wigdets.append(dw)

            obs = [lambda i=i, c=c: c.last_read.stds[i, :]
                   for i in range(c.cam.std_lines)]
            op2 = ObserverPlotWithControls(
                c.cam.std_names, obs, lf, x=c.disp_axis)
            op2.obs_plot.setYRange(0, 8)
            dw = QDockWidget('Readings - stddev', parent=self)
            dw.setWidget(op2)
            dock_wigdets.append(dw)

            obs = [lambda i=i, c=c: c.last_read.signals[i, :]
                   for i in range(c.cam.sig_lines)]
            op3 = ObserverPlotWithControls(
                c.cam.sig_names, obs, lf, x=c.disp_axis)
        ...
        if len(dock_wigdets) > 3:
            self.splitDockWidget(
                dock_wigdets[0], dock_wigdets[3], Qt.Horizontal)
            self.splitDockWidget(
                dock_wigdets[1], dock_wigdets[4], Qt.Horizontal)
            self.splitDockWidget(
    ...
    def setup_toolbar(self):
        ...
        if self.controller.sample_holder is not None:
            plans.append(
                ('Focus Scan', 'fa5s.ruler-combined', FocusScanStarter))
...
class CommandMenu(QWidget):
    ...
    def add_plan_controls(self):
        ...
        pause_plan_but = QPushButton(
            text="Pause plan", icon=qta.icon('fa.pause'))
...
class CamControls(QGroupBox):
    def __init__(self, c: Controller):
        ...
        if c.cam.cam.can_validate_pixel:
            ...
            bg_buttons.append(
                ("Delete valid pix", c.cam.cam.delete_valid_pixel))
...
class SpectrometerControls(QGroupBox):
    def __init__(self, cam: ICam, *args):
        ...
        spec.sigGratingChanged.connect(
            lambda g: lbl.setText('G: %s' % gratings[g]))
...
class DelayLineControl(QGroupBox):
    def __init__(self, c: Controller):
        ...
        dl1c = ControlFactory('Delay 1', lambda x: c.delay_line.set_pos(x, do_wait=False),
                              ...
                              preset_func=lambda x: dl.set_pos(
                                  dl.get_pos() + x, do_wait=False),
                              )

...
from MessPy.QtHelpers import vlay, hlay, PlanStartDialog, ObserverPlot
...
class FocusScanView(QWidget):
    def __init__(self, fsPlan: FocusScan, *args, **kwargs):
        ...
        def plotFit():
            ...
            if fsPlan.scan_x:
                self.x_probe_plot.plotItem.plot(
                    fsPlan.pos_x, fsPlan.fit_xprobe.model, pen=pen)
                text_1 = pg.TextItem(fsPlan.xtext_probe, anchor=(0, 1.0))
                text_1.setPos(fsPlan.pos_x[int(
                    len(fsPlan.pos_x) / 2)], (np.max(fsPlan.probe_x) + np.min(fsPlan.probe_x)) / 2.)
                self.x_probe_plot.plotItem.addItem(text_1)
                self.x_ref_plot.plotItem.plot(
                    fsPlan.pos_x, fsPlan.fit_xref.model, pen=pen)
                text_2 = pg.TextItem(fsPlan.xtext_ref, anchor=(0, 1.0))
                text_2.setPos(fsPlan.pos_x[int(
                    len(fsPlan.pos_x) / 2)], (np.max(fsPlan.ref_x) + np.min(fsPlan.ref_x)) / 2.)
                self.x_ref_plot.plotItem.addItem(text_2)

            if fsPlan.scan_y:
                self.y_probe_plot.plotItem.plot(
                    fsPlan.pos_y, fsPlan.fit_yprobe.model, pen=pen)
                text_3 = pg.TextItem(fsPlan.ytext_probe, anchor=(0, 1.0))
                text_3.setPos(fsPlan.pos_y[int(
                    len(fsPlan.pos_y) / 2)], (np.max(fsPlan.probe_y) + np.min(fsPlan.probe_y)) / 2.)
                self.y_probe_plot.plotItem.addItem(text_3)
                self.y_ref_plot.plotItem.plot(
                    fsPlan.pos_y, fsPlan.fit_yref.model, pen=pen)
                text_4 = pg.TextItem(fsPlan.ytext_ref, anchor=(0, 1.0))
                text_4.setPos(fsPlan.pos_y[int(
                    len(fsPlan.pos_y) / 2)], (np.max(fsPlan.ref_y) + np.min(fsPlan.ref_y)) / 2.)

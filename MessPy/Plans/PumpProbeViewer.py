...
class LineLabel(QLabel):
    ...
    def update_label(self, ev=None):
        x = self.line.pos().x()
        self.setText(
            f'<font size=15 style="bold" color="{self.color}">{self.wl_idx}: {x:.1f}</font>'
        )
...
class PumpProbeDataViewer(QWidget):
    def __init__(self, pp_plan: PumpProbeData, pp: PumpProbePlan, parent=None):
        ...
        self.inf_lines = [list() for _ in pp_plan.cwl]
        self.setup_ui()
        self.setup_plots()
    ...
    def setup_ui(self):
        ...
        vlay.addWidget(self.create_signal_ch_selector())
        vlay.addWidget(
            self.create_checkbox("Current Scan", self.hide_current_lines, True)
        )
        vlay.addWidget(
            self.create_checkbox("Mean of all scans", self.hide_history_lines, True)
        )
        vlay.addWidget(
            self.create_checkbox("Use Wavenumbers", self.update_indicator_line_pos)
        )
    ...
    def create_signal_ch_selector(self):
        ...
        return self.signal_ch_selector

    def create_checkbox(self, text, slot, checked=False):
    ...
    def setup_plots(self):
        ...
        self.trans_plot = ObserverPlot([], self.pp_plan.sigStepDone, aa=True)
        self._layout.itemAt(1).addWidget(self.sig_plot)
        self._layout.itemAt(1).addWidget(self.trans_plot)

    def connect_signals(self):
        for i in [self.update_info, self.update_trans]:
            self.pp_plan.sigStepDone.connect(i)
        self.pp_plan.sigWavelengthChanged.connect(self.handle_wl_change)
        self.sig_plot.plotItem.sigMouseClicked.connect(self.handle_sig_click)
    ...
    def handle_sig_click(self, coords):
        ...
        il = IndicatorLine(
            ...
            wn=1e7 / self.pp_plan.wavelengths,
            pos=x,
            line=l,
            trans_line=tl,
            hist_trans_line=tlh,
    ...
    def update_info(self):
        ...
        s = self.pp_plan
        rot_stage_pos = (
            f"<dt>t pos:<dd>{s.t_idx + 1}/{len(s.t_list)}" if p.rot_stage_angles else ""
        )
        info_text = f"""
    ...
    def hide_history_lines(self, i):
        self.toggle_lines(self.do_show_mean.checkState(), "hist_trans_line")

    def hide_current_lines(self, i):
        self.toggle_lines(self.do_show_cur.checkState(), "trans_line")
        if self.do_show_cur.checkState():
            self.handle_wl_change()

    def toggle_lines(self, state, line_attr):
        all_lines = [line for sublist in self.inf_lines for line in sublist]
        for line in all_lines:
            if state:
                self.trans_plot.addItem(getattr(line, line_attr))
            else:
                self.trans_plot.removeItem(getattr(line, line_attr))
    ...
    def get_x(self):
        ...
        return 1e7 / wl if self.use_wavenumbers.isChecked() else wl
        ...
        if len(self.controller.shutter) > 0 and False:
            ...
            tmp.append(
                dict(
                    name="Pump Shutter",
                    type="list",
                    limits=self.controller.shutter,

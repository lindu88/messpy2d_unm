...
class Dispersion:
    ...
    def calculate_phase(self, nu):
        """
        Calculates the phase for given dispersion terms
        """
        x = nu - self.nu0_THz
        x *= (2 * np.pi)
        factors = np.array([self.GVD, self.TOD, self.FOD]) / np.array([2, 6, 24]) / 1000.
        phase = x ** 2 * factors[0] + x ** 3 * factors[1] + x ** 3 * factors[2]
        return phase


@attr.s(auto_attribs=True)
class AOMRunSettings:
    running: bool = True
    chopped: bool = True
...
class AOM:
    amp_fac: float = 1
    freq_calibration: Optional[tuple] = None

    nu: Optional[np.ndarray] = None
    nu0_THz: float = 55
    t: np.ndarray = np.arange(PIXEL) / 1.2e9
    disp_settings: Dispersion = attr.Factory(Dispersion, 55)
    ...
    unscaled_mask : np.ndarray = np.zeros(PIXEL)
    total_phase: Optional[np.ndarray] = np.zeros_like(PIXEL)
    run_settings: AOMRunSettings = attr.Factory(AOMRunSettings)
    ...
    def set_dispersion_correct(self,
                               GVD: Optional[float] = None,
                               TOD: Optional[float] = None,
                               FOD: Optional[float] = None):
    ...
    def generate_waveform(self, amp=None, phase=None):
        ...
        if self.mode == 'bragg' and self.freq_calibration is not None:
            masks = MAX_16_Bit * self.amp_fac * wf(amp, phase, self.freq_calibration, self.nu0_THz)
        else:
    ...
    def load_full_mask(self):
        amps = np.ones(PIXEL)
        phase = np.zeros(PIXEL)
        self.generate_waveform(amps, phase)
